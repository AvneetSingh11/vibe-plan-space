  ...
  app.post("/api/ai/emotional-insights", async (req, res) => {
    ...
    if (ai) {
      try {
        ...
        const logSummary = emotionLogs && emotionLogs.length > 0 ? emotionLogs.map((l) => `- Logged: ${l.emotion} (${l.emoji}). Stage: ${l.stage}. Energy: ${l.energy}. Pleasantness: ${l.pleasantness}. Note: "${l.note || ""}".`).join("\n") : "No general emotion logs recorded.";
