    ...
    if (ai) {
        try {
            const taskSummary = tasks && tasks.length > 0
                ? tasks.map((t) => `- ${t.title} (${t.urgent ? 'Urgent' : ''} ${t.important ? 'Important' : ''})`).join("\n")
    ...
    if (ai) {
        try {
            const logSummary = emotionLogs && emotionLogs.length > 0
                ? emotionLogs.map((l) => `- Logged: ${l.emotion}. Energy: ${l.energy}. Note: "${l.note || ''}".`).join("\n")
                ...
                config: {
                    ...
                    responseSchema: {
                        ...
                        properties: {
                            ...
                            productivityCorrelation: { type: genai_1.Type.STRING },
                            actionableAdvice: { type: genai_1.Type.STRING }
                        },
                        ...
                        properties: {
                            analysis: {
                                ...
                                properties: {
                                    ...
                                    cognitiveDistortion: { type: genai_1.Type.STRING },
                                    primaryTrigger: { type: genai_1.Type.STRING },
                                    copingStrategy: { type: genai_1.Type.STRING }
                                },
            ...
            const response = await ai.models.generateContent({
                ...
                config: {
                    ...
                    responseSchema: {
                        type: genai_1.Type.OBJECT,
                        properties: {
                            action: { type: genai_1.Type.STRING, enum: ["add_task", "get_briefing", "breakdown", "unknown"] },
                            taskDetails: {
                                type: genai_1.Type.OBJECT,
                                properties: {
                                    title: { type: genai_1.Type.STRING },
                                    urgent: { type: genai_1.Type.BOOLEAN },
                                    important: { type: genai_1.Type.BOOLEAN },
                                },
